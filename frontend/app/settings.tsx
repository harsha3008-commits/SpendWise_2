...
export default function SettingsScreen() {
  ...
  const handleMonthlyReport = async () => {
    try {
      Alert.alert('📊 Generating Monthly Report', 'Creating your personalized financial report...');
      ...
      if (response.ok) {
        ...
        Alert.alert(
          ...
          `Period: ${reportData.period || 'Current Month'}\n\n💰 Total Income: ₹${reportData.total_income || 0}\n💸 Total Expenses: ₹${reportData.total_expenses || 0}\n💵 Net Balance: ₹${reportData.net_balance || 0}\n📈 Transactions: ${reportData.transaction_count || 0}`,
          [
            { text: 'Close' },
            { 
              text: 'Send to WhatsApp', 
              onPress: () => handleWhatsAppReport(reportData)
            }
          ]
        );
      } else {
  ...
  const handleWhatsAppReport = (reportData: any) => {
    const reportText = `
🏦 SpendWise Monthly Report
📅 ${reportData.period || 'Current Month'}

💰 Income: ₹${reportData.total_income || 0}
💸 Expenses: ₹${reportData.total_expenses || 0}
💵 Net Balance: ₹${reportData.net_balance || 0}
📊 Total Transactions: ${reportData.transaction_count || 0}

${reportData.ai_summary || 'AI insights not available'}

Generated by SpendWise - Your Smart Finance Tracker
    `.trim();
      
    Alert.alert(
      '📱 Send Report to WhatsApp',
      'Your financial report is ready to share. This will open WhatsApp with your report text.',
      [
        { text: 'Cancel', style: 'cancel' },
        { 
          text: 'Send to WhatsApp', 
          onPress: () => {
            // In a real app, this would use a WhatsApp API or deep link
            Alert.alert(
              '✅ Report Ready!',
              `Your report has been prepared:\n\n${reportText.substring(0, 200)}...\n\nIn the full app version, this would automatically send to WhatsApp.`,
              [{ text: 'OK' }]
            );
          }
        }
